...
Just a summary without learnings.
  ...
  describe('parseFollowUpResponse', () => {
    it('extracts bullet points from response', () => {
      const response = `
- First insight
- Second insight
- Third insight
...
3. Third point
    ...
    it('returns whole response if short and no bullets', () => {
      const response = 'This is a single paragraph response.';

      const result = parseFollowUpResponse(response);

      expect(result).toHaveLength(1);
      expect(result[0]).toBe('This is a single paragraph response.');
    });

    it('splits long non-bullet response into sentences', () => {
      // Response must be > 500 chars to trigger sentence splitting
      // Each sentence needs > 20 chars and must be separated by ". "
      const sentences = [
      ...
      // Verify response is long enough (must be > 500)
    ...
    it('handles mixed format with bullets', () => {
      const response = `
